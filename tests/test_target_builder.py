...
from r5.ssl.target_builder import build_set_valued_targets
...
def test_r5_sam_foreground_support_does_not_create_background_hard_label():
    ...
    assert targets["stats"]["per_class_sam_participation_ratio"][0] == 0.0


def test_r5_emergency_mode_disables_background_when_foreground_absent():
    cal = PromptReliabilityCalibrator(3, min_pixels_per_class=1, use_soft_gate=True)
    cal.teacher_q = torch.tensor([0.50, 0.50, 0.50])
    cal.sam_q = torch.tensor([0.50, 0.50, 0.50])
