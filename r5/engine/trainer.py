...
from r5.calibration import PromptReliabilityCalibrator, SAMUtilityScheduler
from r5.data.dataset_2d import SegmentationDataset2D, resolve_dataset_root
from r5.data.paired_sampler import paired_batches
from r5.data.split import create_train_calibration_split
from r5.engine.checkpoint import export_deploy_payload, safe_load, save_checkpoint
from r5.engine.evaluator import evaluate
from r5.engine.logger import OneLineProgress, append_jsonl, setup_logger
from r5.engine.model_factory import build_deploy_model
from r5.losses.boundary_losses import boundary_bce_loss
from r5.losses.foreground_safe_kd import foreground_safe_sam_consistency_loss, foreground_safe_sam_kd_loss
from r5.losses.sam_adapter_losses import sam_ce_dice_loss
from r5.losses.tri_state_pseudo_loss import tri_state_pseudo_supervision_loss
from r5.losses.supervised import supervised_loss
from r5.models.dual_temporal_teacher import DualTemporalTeacher
from r5.models.promptable_sam_mentor import PromptableSAMMentor
...
from r5.ssl.online_sam_relation import online_sam_student_relation_loss
from r5.ssl.foreground_safe_target_builder import build_foreground_safe_targets
...
class SAGESAMR5Trainer:
    ...
    def _r5_stage_weights(self, iteration: int, fast_slow_agreement: float):
        cfg = self.config.get("r5", {})
    ...
    def train_one_iter(self, batch_l, batch_u, iteration: int, update: bool = True, step_optimizer: bool = True):
        ...
        stage_weights = self._r5_stage_weights(iteration, float(teacher_out["agreement"].detach()))

        pseudo_cfg = self.config.get("pseudo", {})
        pseudo_runtime_cfg = dict(self.config.get("r5", {}))
        ...
        with amp_autocast(self.device.type, self.amp):
            ...
            if self.use_sam and sam_u.get("valid"):
                foreground_mask = targets["foreground_seed_mask"].bool()
                ...
                if structure_cfg.get("use_online_relation", True):
                    loss_relation = online_sam_student_relation_loss(
                        ...
                        gate=targets["structure_gate"] & (targets["foreground_seed_mask"] | targets["fuzzy_region"]),
        ...
        logs = {
            ...
            "unsup_weight": ramp,
            "r5_unsup_scale": float(unsup_scale),
            "r5_stage": float(stage_weights["stage"]),
            "r5_low_agreement": float(stage_weights["low_agreement"]),
