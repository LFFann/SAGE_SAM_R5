...
def _find_local_R5_root() -> Path | None:
    here = Path(__file__).resolve()
    for root in [here.parents[3], Path.cwd() / "SAGE_SAM_R5"]:
...
class RealSAMWrapper(nn.Module):
    ...
    def _build_sam(self, model_type: str):
        local_root = _find_local_R5_root()
        ...
        try:
            ...
            raise ImportError(
                "sam.use_sam=true requires either SAGE_SAM_R5/Model/sam copied from KnowSAM or the segment_anything package."
            ) from exc
    ...
    def _build_local_knowsam_sam(self, local_root: Path, model_type: str):
        ...
        self.sam_source = f"SAGE_SAM_R5/Model/sam:{local_root}"
        return sam.to(self.device)
    ...
    def _prepare_sparse_prompts(self, prompts: dict[str, torch.Tensor]):
        local_prompt_encoder = str(self.sam_source or "").startswith("SAGE_SAM_R5/Model/sam")
